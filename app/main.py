...
async def check_for_reply(thread_id: str):
    ...
    try:
        ...
        if new_replies:
            ...
            # Check if auto-negotiate mode is enabled
            is_auto = current_state.values.get("auto_negotiate", False)
            auto_sent = False
            
            if is_auto and agent_response.get("message"):
                # AUTO MODE: Send the agent's response immediately without human approval
                logger.info(f"🤖 AUTO-NEGOTIATE: Auto-sending response: {agent_response['message']}")
                
                from app.messaging.service import get_messaging_service
                
                # Resolve target number (same logic as send-chat)
                ...
                target_number = (
                    normalize_phone_auto(settings.default_target_number)
                    or normalize_phone_auto(place_phone)
                    or normalize_phone_auto(settings.twilio_target_number)
                )
                
                if target_number:
                    messaging_auto = get_messaging_service()
                    sms_sent = messaging_auto.send_message(to=target_number, message=agent_response["message"])
                    
                    if sms_sent:
                        logger.info(f"🤖 AUTO-NEGOTIATE: SMS sent to {target_number}")
                        auto_sent = True
                        
                        # Update history with agent's auto-sent message
                        auto_history = new_history + [{
                            "role": "agent",
                        ...
                        await negotiator_agent.aupdate_state(config, {
                            "negotiation_history": auto_history,
                            "negotiation_status": "ongoing"
                        })
                    else:
                        logger.error("🤖 AUTO-NEGOTIATE: Failed to send SMS")
...
async def send_manual_message(request: dict):
    ...
    try:
        ...
        recs = state_values.get("recommendations") or state_values.get("serp_results") or []
        place_phone = recs[0].get("phone") if recs and isinstance(recs[0], dict) else ""
        # During testing: prefer DEFAULT_TARGET_NUMBER; in production: use shop's phone
        target_number = (
            normalize_phone(settings.default_target_number)
            or normalize_phone(place_phone)
...
# ==========================================
# ENDPOINT: TOGGLE AUTO-NEGOTIATE MODE
# ==========================================

@app.post("/agent/toggle-auto")
async def toggle_auto_negotiate(request: dict):
    """
    Toggle auto-negotiate mode on/off mid-conversation.
    Updates the auto_negotiate flag in agent state.
    """
    try:
        thread_id = request.get("thread_id")
        auto_negotiate = request.get("auto_negotiate", False)
        
        if not thread_id:
            raise HTTPException(status_code=400, detail="thread_id required")
        
        logger.info(f"🔄 Toggle auto-negotiate: {auto_negotiate} for thread {thread_id}")
        ...
        return {"success": True, "auto_negotiate": auto_negotiate}
        
    except Exception as e:
        logger.error(f"❌ Toggle auto error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def start_negotiation(request: NegotiationStartRequest):
    ...
    try:
        ...
        negotiation_state = {
            ...
            "auto_negotiate": request.auto_negotiate,  # Auto-negotiate mode flag
        ...
        logger.info(f"📝 Updating state for negotiation... (auto_negotiate={request.auto_negotiate})")
        ...
        if next_node and "human_review" in next_node:
            ...
            if request.auto_negotiate:
                # AUTO MODE: Skip HITL — auto-approve and send SMS immediately
                logger.info("🤖 AUTO-NEGOTIATE: Skipping HITL, auto-approving...")
                
                await negotiator_agent.aupdate_state(config, {
                    "human_approved": True
                })
                
                # Resume graph execution (sends SMS)
                auto_result = await negotiator_agent.ainvoke(None, config)
                
                logger.info("🤖 AUTO-NEGOTIATE: SMS sent automatically!")
                
                # Return as if chat should open directly
                return AgentResponse(
                    message="Auto-negotiation started - SMS sent automatically",
                    thread_id=request.thread_id,
                    data={
                        ...
                        "requires_approval": False,
                        "auto_negotiate": True,
                        "negotiation_status": "end"
                    },
                    requires_approval=False,
                    is_complete=True
                )
            else:
                # MANUAL MODE: Show HITL panel for approval
        ...
        return AgentResponse(
            ...
            data={
                ...
                "target_place": target_place,
                "requires_approval": requires_approval,
                "auto_negotiate": request.auto_negotiate
            },
            requires_approval=requires_approval,
            is_complete=False
        )
