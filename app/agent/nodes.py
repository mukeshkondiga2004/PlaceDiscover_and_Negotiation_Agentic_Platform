...
async def human_review_node(state: AgentState) -> dict:
    ...
    if state.get("human_approved"):
        ...
        # Normalize phone numbers: keep digits, add +91 for Indian numbers
        ...
        # During testing: prefer DEFAULT_TARGET_NUMBER; in production: use shop's phone from SerpStack
        target_number = normalize_phone(settings.default_target_number) or normalize_phone(place_phone) or normalize_phone(settings.twilio_target_number)
...
async def review_extraction_node(state: AgentState) -> dict:
    ...
    if not recommendations:
        ...
        return {
            ...
            "reviews": {},
            "is_complete": True,
            "messages": state.get("messages", []) + [AIMessage(content="❌ No places to fetch reviews for.")]
        }
...
async def analyze_reviews_node(state: AgentState) -> dict:
    ...
    if not recommendations:
        return {
            ...
            "is_complete": True,
            "messages": state.get("messages", []) + [AIMessage(content="❌ No places to analyze.")]
        }
